...
function shoppingTime(memberId, money) {
  // you can only write your code here!
    ...
    if(memberId == undefined && money == undefined) return "Mohon maaf, toko X hanya berlaku untuk member saja";

    let changeMoney = money;
    for(const key in ObjectBarang){
        if(ObjectBarang[key] <= money) {
            changeMoney = changeMoney - ObjectBarang[key];
            list_belanjaan.push(key);
        }
    }
